...
class EvomanAnnNeat(EvomanSimple):
    """
    Class that implement the NEAT algorithm using an ANN architecture.
    """
    ...
    def do_train(self, simulate):
        ...
        if "player" in self.train_controllers:
            config_file = "../z_configuration_files/z_evoman_ann_neat_player.cfg"
        ...
        if "enemy" in self.train_controllers:
            config_file = "../z_configuration_files/z_evoman_ann_neat_enemy.cfg"
        ...
        if len(population) == 1:
            ...
            for reporter in thisPopulation.reporters.reporters:
                if isinstance(reporter, StatisticsReporter):
                    ...
                    reporter.save_species_fitness(delimiter=';',
                                                  filename=self.statistics_dir + '/fitness_by_species.csv')
                    reporter.save_species_count(delimiter=';', filename=self.statistics_dir + '/speciation.csv')
...
if __name__ == "__main__":
    ...
    if run_mode == "train":
        ea = EvomanAnnNeat(num_generations=10, statistics_dir=base_dir + '/stats')
        ea.train(simulate)
